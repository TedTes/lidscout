-- Add scan-ready public sources for the ecommerce fulfillment template.
--
-- The original template had only one enabled gate-free source (Hacker News).
-- Review and Reddit sources remain valuable, but they are intentionally disabled
-- until proxy/OAuth support is configured. These additions use JSON APIs that
-- the current worker can scan without auth.
...
with source_seed (
    ...
    values
        (
            'https://hn.algolia.com/api/v1/search_by_date?query=shipbob+fulfillment&tags=comment',
            'hackernews',
            'technical_forum',
            true,
            'api',
            false,
            false,
            '{"adapter": "json", "items_path": "hits", "source_family": "technical_forum"}'::jsonb,
            2,
            0.78,
            'daily',
            25
        ...
        (
            'https://api.github.com/search/issues?q=shipstation+shipping+is%3Aissue&sort=updated&order=desc',
        ...
        (
            'https://api.github.com/search/issues?q=easypost+shipping+is%3Aissue&sort=updated&order=desc',
            'github_issues_search',
            'technical_forum',
            true,
            'api',
        ...
        (
            'https://api.stackexchange.com/2.3/search/advanced?q=ecommerce+shipping+labels+fulfillment&site=stackoverflow&order=desc&sort=creation&pagesize=25',
        ...
        (
            'https://api.stackexchange.com/2.3/search/advanced?q=shipstation+api+shipping+labels&site=stackoverflow&order=desc&sort=creation&pagesize=25',
            'stackoverflow_search',
            'technical_forum',
            true,
            'api',
...
fulfillment_template as (
    select id
    from niches
    where job = 'Manage ecommerce fulfillment and shipping'
      and is_custom = false
    limit 1
),
